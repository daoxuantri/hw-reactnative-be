...
const sendOTP = async({email, subject, message, duration = 5}) =>{
    try{
        ...
        return {
            ...createdOTPRecord.toObject(),  // Convert MongoDB document to a plain JS object
            success: true
        };
    }catch(e){
        throw e;
...
//flutter
const sendVerificationOTPEmaill = async (email) => {
    try{
        //check if an account exists
        const existingUser = await User.findOne({email});
        if(!existingUser){
...
module.exports = { sendOTP, verifyOTP , sendVerificationOTPEmail, verifyUserEmail, deleteOTP, sendVerificationOTPEmaill};